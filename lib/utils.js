...
exports.getReadableCoins = function (coins, digits, withoutSymbol) {
	...
	// Handle multipls currencies from Haven
	let amount_str = "";
	Object.keys(coins)
	    .forEach(function(asset_type) {
		let amount = (parseInt(coins[asset_type] || 0) / config.coinUnits)
		    .toFixed(digits || coinDecimalPlaces);
		if (amount_str.length) amount_str += ",";
		amount_str += amount + " " + asset_type;
	    });
	return amount_str;
    }
}
