			...
			async.filter(blocks, function (block, mapCback) {
				...
				apiInterfaces.rpcDaemon(rpcMethod, {
					...
					block.unlocked = blockHeader.depth >= config.blockUnlocker.depth
					    ...
					    block.reward = blockHeader.rewards
					} else {
		...
		function (blocks, callback) {
			...
			blocks.forEach(function (block) {
				...
				unlockedBlocksCommands.push(['del', config.coin + ':shares_actual:prop:round' + block.height])
			        ...
			        let block_reward_str = block.reward
			        if (config.coin == "haven") {
				    // Handle all of the currencies
				    block_reward_str = "";
				    block.reward.forEach(function(asset) {
					block_reward_str += (block_reward_str.length) ? "," : "";
					block_reward_str += asset.asset_type + ":" + asset.amount;
				    });
				}
				...
				if (Object.keys(donations)
					...
					for (let wallet in donations) {
					    ...
					    feePercent += percent
					    if (config.coin == "haven") {
						Object.keys(block.reward)
						    .forEach(function(asset_type) {
							payments[wallet][asset_type] = block.reward[asset_type] * percent
						    })
						log('info', logSystem, 'Block %d donation to %s as %d percent of reward: %s', [block.height, wallet, percent, JSON.stringify(payments[wallet])])
					    } else {
				...
				let reward = Math.round(block.reward - (block.reward * feePercent))
				    ...
				    reward = {}
				    block.reward.forEach(function(asset) {
					reward[asset.asset_type] = asset.amount - (asset.amount * feePercent)
				    });
				    log('info', logSystem, 'Unlocked %d block with reward %j and donation fee %d. Miners reward: %s', [block.height, block.reward, feePercent, JSON.stringify(reward)])
				} else {
				...
				if (block.workerScores) {
					...
					if (block.rewardType === 'solo') {
						...
						Object.keys(block.reward)
						    .forEach(function(asset_type) {
							payments[worker][asset_type] = (payments[worker][asset_type] || 0) + reward[asset_type]
						    })
						log('info', logSystem, 'SOLO Block %d payment to %s for %d%% of total block score: %s', [block.height, worker, 100, JSON.stringify(payments[worker])])
					    } else {
						...
						Object.keys(block.workerScores)
							.forEach(function (worker) {
								...
								block.reward.forEach(function(asset) {
									...
									let workerReward = Math.round(reward[asset.asset_type] * percent)
									payments[worker][asset.asset_type] = (payments[worker][asset.asset_type] || 0) + workerReward
									log('info', logSystem, 'PROP Block %d payment to %s for %d%% of total block score: %s', [block.height, worker, percent * 100, JSON.stringify(payments[worker])])
								    })
							    } else {
								let percent = block.workerScores[worker] / totalScore
			    ...
			    if (config.coin == "haven") {
				Object.keys(payments[worker])
				    .forEach(function(asset_type) {
					let amount = parseInt(payments[worker][asset_type])
					log('error', logSystem, 'PAYMENTS[WORKER] = %j, AMOUNT = %d', [payments[worker], amount]);
					if (amount <= 0) {
					    delete payments[worker][asset_type]
					    if (payments[worker].keys.length == 0) {
					...
					if (asset_type === "XHV") {
					    ...
					    unlockedBlocksCommands.push(['hincrby', `${config.coin}:workers:${worker}`, 'balance-'+asset_type, amount]);
					}
				    });
			    } else {
			...
			if (unlockedBlocksCommands.length === 0) {
		        ...
		        log('info', logSystem, 'unlockedBlocksCommands = %j', [unlockedBlocksCommands]);
