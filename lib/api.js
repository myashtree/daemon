		...
		config: function (callback) {
			callback(null, {
				...
				paymentsInterval: config.payments.interval,
			        minPaymentThreshold: (config.coin == "haven") ? config.payments.assets["XHV"].minPayment : config.payments.minPayment,
			        maxPaymentThreshold: ((config.coin == "haven") ? config.payments.assets["XHV"].maxPayment : config.payments.maxPayment) || null,
				transferFee: (config.coin == "haven") ? config.payments.assets["XHV"].transferFee : config.payments.transferFee,
				denominationUnit: (config.coin == "haven") ? config.payments.assets["XHV"].denomination : config.payments.denomination,
...
function handleGetMinerPayoutLevel (urlParts, response) {
	...
	response.write('\n');
        ...
        let address = urlParts.query.address;
        let asset = urlParts.query.asset;
	...
	if (address === undefined) {
        ...
        // Check for an asset being specified
        if (config.coin == "haven") {
	    if (asset === undefined) asset = "XHV";

	    // Return current miner payout level
	    redisClient.hget(`${config.coin}:workers:${address}`, (asset == "XHV") ? 'minPayoutLevel' : 'minPayoutLevel-' + asset, function (error, value) {
		if (error) {
			response.end(JSON.stringify({
	        ...
	        let minLevel = config.payments.assets[asset].minPayment / config.coinUnits;
		if (minLevel < 0) minLevel = 0;

	        let maxLevel = config.payments.assets[asset].maxPayment ? config.payments.assets[asset].maxPayment / config.coinUnits : null;
...
function handleSetMinerPayoutLevel (urlParts, response) {
	...
	minerSeenWithIPForAddress(address, ip, function (error, found) {
		...
		let payoutLevel = level * config.coinUnits;
	        redisClient.hset(config.coin + ':workers:' + address, (asset == "XHV") ? 'minPayoutLevel' : 'minPayoutLevel-' + asset, payoutLevel, function (error, value) {
			if (error) {
				response.end(JSON.stringify({
